(function () {
  ...
  function fillSettings() {
    ...
    $("settingLatestVersion").value = asString(state.config.latestVersion, "2.6.0");
    $("settingMinVersion").value = asString(state.config.minimumVersion, "2.6.0");
  ...
  async function saveSettings(event) {
    ...
    setBusy(form, true);
    try {
      const latestVersion = asString($("settingLatestVersion").value, "2.6.0").slice(0, 20);
      const versionChanged = latestVersion !== asString(state.config.latestVersion, "2.6.0");
      ...
      const payload = {
        ...
        latestVersion,
        minimumVersion: asString($("settingMinVersion").value, "2.6.0").slice(0, 20),
  ...
  if (!window.CinaroNative && "serviceWorker" in navigator && location.protocol === "https:") {
    navigator.serviceWorker.register("sw.js?v=2.6.0", { scope: "./", updateViaCache: "none" }).catch((error) => console.warn("CINARO admin service worker unavailable", error));
  }
})();
