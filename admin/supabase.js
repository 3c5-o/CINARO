...
const configFromRow = (row) => ({
  ...
  latestVersion: row?.latest_version || "2.8.0",
  minimumVersion: row?.minimum_version || "2.8.0",
...
async function saveDocument(name, id, payload) {
  ...
  if (name === "appConfig") {
    ...
    const row = {
      ...
      latest_version: patch.latestVersion !== undefined ? clean(patch.latestVersion, "2.8.0", 20) : old.latest_version || "2.8.0",
      minimum_version: patch.minimumVersion !== undefined ? clean(patch.minimumVersion, "2.8.0", 20) : old.minimum_version || "2.8.0",
