...
const configFromRow = (row) => ({
  ...
  latestVersion: row?.latest_version || "2.7.1",
  minimumVersion: row?.minimum_version || "2.7.1",
...
async function saveDocument(name, id, payload) {
  ...
  if (name === "appConfig") {
    ...
    const row = {
      ...
      latest_version: patch.latestVersion !== undefined ? clean(patch.latestVersion, "2.7.1", 20) : old.latest_version || "2.7.1",
      minimum_version: patch.minimumVersion !== undefined ? clean(patch.minimumVersion, "2.7.1", 20) : old.minimum_version || "2.7.1",
