...
assert(packageJson.version === "2.5.0", "package version is 2.5.0");
...
assert(webHtml.includes('supabase.js?v=2.5.0'), "user shell loads Supabase 2.5 client");
assert(adminHtml.includes('supabase.js?v=2.5.0'), "admin shell loads Supabase 2.5 client");
...
assert(webApp.includes('const WEB_APP_VERSION = "2.5.0"'), "user runtime version is 2.5.0");
...
assert(webStyles.includes("CINARO 2.5"), "new user design system is present");
assert(adminStyles.includes("CINARO Admin 2.5"), "new admin design system is present");
assert(webSw.includes('cinaro-v2.5.0'), "user PWA cache bumped");
assert(webSw.includes('"./supabase.js"'), "user PWA caches Supabase client");
assert(adminSw.includes('cinaro-admin-v2.5.0'), "admin PWA cache bumped");
assert(adminSw.includes('"./supabase.js"'), "admin PWA caches Supabase client");

assert(buildGradle.includes("versionCode 12"), "Android versionCode bumped");
assert(buildGradle.includes('versionName "2.5.0"'), "Android user version is 2.5.0");
assert(buildGradle.includes('versionName "2.5.0-admin"'), "Android admin version is 2.5.0-admin");
assert(buildGradle.includes("?v=2.5.0#home"), "user APK URL carries native release version");
assert(buildGradle.includes("?v=2.5.0#dashboard"), "admin APK URL carries native release version");

assert(workflow.includes("CINARO-User-v2.5.0.apk"), "workflow names user APK 2.5.0");
assert(workflow.includes("CINARO-Admin-v2.5.0.apk"), "workflow names admin APK 2.5.0");
assert(workflow.includes('TAG="v2.5.0"'), "workflow publishes v2.5.0");
